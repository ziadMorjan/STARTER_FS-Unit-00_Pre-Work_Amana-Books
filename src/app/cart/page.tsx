...
export default function CartPage() {
  ...
  // Use a map for O(1) lookup of book details by bookId (which is the MongoDB _id)
  ...
  const loadAndFetchCart = useCallback(async () => {
    ...
    try {
      // Fetch details for all unique books concurrently from the API
      const detailPromises = uniqueBookIds.map(id => 
        fetcher(`/api/books/${id}`)
      );
      
      const details = await Promise.all(detailPromises);
      
      // Map the details by their ID (_id is prioritized)
      const detailsMap: Record<string, Book> = details.reduce((acc, book) => {
        // Use the ID (book._id or book.id) that matches the bookId stored in CartItem
  ...
  useEffect(() => {
    ...
    // Set up event listener for when cart is updated externally (e.g., from BookDetailPage)
  ...
  // Handle quantity change (updates local storage and state)
  ...
  const handleRemoveItem = (cartItemId: string) => {
    ...
    setCartItems(updatedCart);
    localStorage.setItem('cart', JSON.stringify(updatedCart));
    // Trigger a reload to potentially remove unused book details
    loadAndFetchCart(); 
  };
  ...
  const cartDisplayItems = cartItems.map(item => {
    ...
    if (!book) {
      // Handle case where book details could not be fetched or book was deleted
      return { 
        ...item, 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          {cartDisplayItems.map(item => (
            <div 
              key={item.id} // CartItem ID is a unique local ID
            ...
            >
              ...
              <div className="flex-grow">
                {item.found ? (
                  <>
                    <Link href={`/book/${item.bookId}`} className="text-lg font-semibold text-gray-800 hover:text-teal-600 transition-colors">
                      {item.bookTitle}
                    </Link>
                    <p className="text-sm text-gray-600 mb-1">by {item.author}</p>
                    <p className="text-md font-bold text-teal-600">${item.price.toFixed(2)}</p>
                  </>
                ) : (
                  <p className="text-lg font-semibold text-red-500">{item.bookTitle} (Not Available)</p>
                )}
              </div>
